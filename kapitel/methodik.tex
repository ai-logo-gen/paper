...
The foundation for training is the \texttt{iamkaikai/amazing\_logos\_v4} dataset \cite{iamkaikai_amazing_logos_v4}, a multimodal corpus of approximately 400,000 logo-caption pairs. This dataset was chosen for its scale and detailed metadata, which includes company names, descriptions, and stylistic tags.

\subsection{Model Selection and Architecture}\label{sec:modellauswahl}
\subsubsection*{Base Model}
Stable Diffusion v1.5 \cite{HuggingFace_StableDiffusionv15_ModelCard} was selected as the base model. Its open-source nature, extensive ecosystem, and balance between generative quality and computational requirements make it ideal for iterative optimization on consumer hardware (NVIDIA RTX 5080, 16GB VRAM).

\subsubsection*{Structural Control: The ControlNet Pipeline}
To enforce structural fidelity to user sketches, this work utilizes ControlNet \cite{ZHANG2023}. Three pretrained models were evaluated: Canny, Scribble, and Lineart.
\begin{itemize}
...
\begin{table}[hbt!]
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{0.45\textwidth}>{\raggedright\arraybackslash}p{0.45\textwidth}}
        \toprule
...
To ensure the dataset reflects the target aesthetic, a weighted "Minimalism Score" (0-100) was developed based on features such as color count, edge density, contour complexity, and whitespace ratio.
\begin{figure}[H]
    \centering
...
\end{figure}
As shown in Figure \ref{fig:minimalism_score_distribution}, the dataset was filtered using the median score (82.2) as a threshold. From the remaining pool, a class-balanced subset of 1,810 images was extracted to facilitate efficient hyperparameter optimization \cite{ZHANG2023, ruiz2023dreamboothfinetuningtexttoimage}.
...
\begin{enumerate}
    \item \textbf{Sketch Generation:} Stable Diffusion v1.5 with the Scribble ControlNet was used to transform original logos into abstract, hand-drawn-style sketches. The Scribble model was chosen here to simulate the imperfection of human drawings.
...
Low-Rank Adaptation (LoRA) \cite{HU2021} is employed to fine-tune the UNet component of Stable Diffusion. The VAE, Text Encoder, and ControlNet are frozen to preserve their pretrained capabilities and ensure resource efficiency.
The following hyperparameters are systematically analyzed to identify the optimal configuration:
\begin{itemize}
    \item \textbf{Target Modules:} Comparing adaptation of only Attention layers vs. an Extended configuration (Attention + Feed-Forward layers) \cite{dettmers2023qloraefficientfinetuningquantized}.
    \item \textbf{LoRA Rank ($r$):} Evaluating ranks \textbf{\{4, 8, 16, 32\}} to find the balance between expressivity and efficiency \cite{HU2021}.
    \item \textbf{LoRA Alpha ($\alpha$):} Fixed at $1.5 \times r$. This decision is based on the finding of \citet[p. 4]{HU2021} that the effective strength of LoRA adaptation can be primarily controlled via the learning rate, making a separate, detailed tuning of $\alpha$ less critical.
    \item \textbf{Learning Rate:} Exploring the range \textbf{$1 \times 10^{-6}$ to $1 \times 10^{-4}$}, as PEFT methods typically tolerate higher rates than full fine-tuning \cite{zhang2025parameter}.
    \item \textbf{Batch Size:} Fixed at \textbf{8} to maximize hardware utilization.
\end{itemize}
In total, 20 distinct hyperparameter combinations are evaluated (see Appendix \ref{app:hyperpara_kombis} for the complete list)