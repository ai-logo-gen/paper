...
The selected model was evaluated on an unseen test set to assess generalization. To ensure statistical robustness, the evaluation was conducted using five different random seeds, and the results are reported as mean $\pm$ standard deviation. Table \ref{tab:testresults} compares the base model (without and with ControlNet) against the fine-tuned model.
...
\begin{itemize}
    ...
    \item \textbf{Fine-tuning} restores semantic alignment (+3.6\% CLIP) and drastically improves image quality (-31.0\% FID). The significant FID reduction ($228.3$ to $157.6$) outweighs the minor 3.8\% SSIM decrease ($0.819$ to $0.788$), representing a conscious trade-off: minimal structural precision is sacrificed for enhanced semantic and visual quality.
\end{itemize}
